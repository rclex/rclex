...
*/
  ...
  printf("wrold\n");
  *res = rcl_wait_set_init(res_waitset,number_of_subscriptions,number_of_guard_conditions,number_of_timers,number_of_clients,
                            number_of_services,number_of_events,res_context,rcl_get_default_allocator());
  printf("exit from wait_set_init\n");
  return ret;
}
...
ERL_NIF_TERM nif_rcl_wait_set_add_subscription(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]){
  ...
  size_t* index;  //indexはNIF関数内埋め込み,Elixirでポインタよくわからんし
printf("5\n");
  //以下でコンパイルエラー
  if(enif_get_resource(env,argv[0],rt_waitset,(void**) &res_waitset)){
    return enif_make_badarg(env);
  ...
  enif_release_resource(res);
  printf("7\n");
  *res = rcl_wait_set_add_subscription(res_waitset,res_sub,index);
