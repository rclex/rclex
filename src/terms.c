...
ERL_NIF_TERM atom_keep_last; // Only store up to a maximum number of samples, dropping oldest once max is exceeded.
ERL_NIF_TERM atom_keep_all; // Store all samples, subject to resource limits.
ERL_NIF_TERM atom_transient_local; // The rmw publisher is responsible for persisting samples for “late-joining” subscribers.
...
ERL_NIF_TERM atom_best_effort; // Attempt to deliver samples, but some may be lost if the network is not robust.
ERL_NIF_TERM atom_automatic; // The signal that establishes a Topic is alive comes from the ROS rmw layer.
ERL_NIF_TERM atom_manual_by_topic; // The signal that establishes a Topic is alive is at the Topic level. Only publishing a message on the Topic or an explicit signal from the application to assert liveliness on the Topic will mark the Topic as being alive.
