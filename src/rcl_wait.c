...
ERL_NIF_TERM nif_rcl_wait_set_init_client(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  rc = rcl_wait_set_init(&wait_set, 0, 0, 0, 1, 0, 0, context_p, allocator);
  if (rc != RCL_RET_OK) return raise(env, __FILE__, __LINE__);
...
ERL_NIF_TERM nif_rcl_wait_set_init_service(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  rc = rcl_wait_set_init(&wait_set, 0, 0, 0, 0, 1, 0, context_p, allocator);
  if (rc != RCL_RET_OK) return raise(env, __FILE__, __LINE__);
...
ERL_NIF_TERM nif_rcl_wait_client(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (!enif_get_resource(env, argv[2], rt_rcl_client_t, (void **)&client_p))
    return enif_make_badarg(env);
  ...
  rc = rcl_wait_set_add_client(wait_set_p, client_p, NULL);
  if (rc != RCL_RET_OK) return raise(env, __FILE__, __LINE__);
...
ERL_NIF_TERM nif_rcl_wait_service(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (!enif_get_resource(env, argv[2], rt_rcl_service_t, (void **)&service_p))
    return enif_make_badarg(env);
  ...
  rc = rcl_wait_set_add_service(wait_set_p, service_p, NULL);
  if (rc != RCL_RET_OK) return raise(env, __FILE__, __LINE__);
