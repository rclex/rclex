...
ERL_NIF_TERM nif_rosidl_get_geometry_msgs_msg_twist_type_support(ErlNifEnv *env, int argc,
...
ERL_NIF_TERM nif_geometry_msgs_msg_twist_create(ErlNifEnv *env, int argc,
                                                const ERL_NIF_TERM argv[]) {
  ignore_unused(argv);
...
ERL_NIF_TERM nif_geometry_msgs_msg_twist_destroy(ErlNifEnv *env, int argc,
                                                 const ERL_NIF_TERM argv[]) {
...
ERL_NIF_TERM nif_geometry_msgs_msg_twist_set(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (!enif_get_tuple(env, tuple[0], &linear_arity, &linear_tuple)) return enif_make_badarg(env);

  double linear_x;
  if (!enif_get_double(env, linear_tuple[0], &linear_x)) return enif_make_badarg(env);
  message_p->linear.x = linear_x;

  double linear_y;
  if (!enif_get_double(env, linear_tuple[1], &linear_y)) return enif_make_badarg(env);
  message_p->linear.y = linear_y;

  double linear_z;
  if (!enif_get_double(env, linear_tuple[2], &linear_z)) return enif_make_badarg(env);
...
ERL_NIF_TERM nif_geometry_msgs_msg_twist_get(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  return enif_make_tuple(env, 2,
                         enif_make_tuple(env, 3, enif_make_double(env, message_p->linear.x),
                                         enif_make_double(env, message_p->linear.y),
                                         enif_make_double(env, message_p->linear.z)),
                         enif_make_tuple(env, 3, enif_make_double(env, message_p->angular.x),
