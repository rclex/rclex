...
ERL_NIF_TERM nif_sensor_msgs_msg_point_cloud_set(ErlNifEnv *env, int argc,
  ...
  if (!enif_get_tuple(env, tuple[0], &header_arity, &header_tuple)) return enif_make_badarg(env);
  ...
  if (!enif_get_int(env, header_stamp_tuple[0], &(header_stamp_sec))) return enif_make_badarg(env);
  ...
  if (enif_get_string(env, header_tuple[1], header_frame_id, header_frame_id_length + 1,
                      ERL_NIF_LATIN1) <= 0)
    return enif_make_badarg(env);
  ...
  if (!enif_get_list_length(env, tuple[1], &points_length)) return enif_make_badarg(env);

  geometry_msgs__msg__Point32__Sequence *points =
      geometry_msgs__msg__Point32__Sequence__create(points_length);
  ...
  for (points_i = 0, points_left = tuple[1]; points_i < points_length;
       ++points_i, points_left   = points_tail) {
    if (!enif_get_list_cell(env, points_left, &points_head, &points_tail))
      return enif_make_badarg(env);

    int points_arity;
    const ERL_NIF_TERM *points_tuple;
    if (!enif_get_tuple(env, points_head, &points_arity, &points_tuple))
      return enif_make_badarg(env);

    double points_i_x;
    if (!enif_get_double(env, points_tuple[0], &points_i_x)) return enif_make_badarg(env);
    message_p->points.data[points_i].x = (float)points_i_x;

    double points_i_y;
    if (!enif_get_double(env, points_tuple[1], &points_i_y)) return enif_make_badarg(env);
    message_p->points.data[points_i].y = (float)points_i_y;

    double points_i_z;
    if (!enif_get_double(env, points_tuple[2], &points_i_z)) return enif_make_badarg(env);
    message_p->points.data[points_i].z = (float)points_i_z;
  }

  unsigned int channels_length;
  if (!enif_get_list_length(env, tuple[2], &channels_length)) return enif_make_badarg(env);

  sensor_msgs__msg__ChannelFloat32__Sequence *channels =
      sensor_msgs__msg__ChannelFloat32__Sequence__create(channels_length);
  ...
  for (channels_i = 0, channels_left = tuple[2]; channels_i < channels_length;
       ++channels_i, channels_left   = channels_tail) {
    if (!enif_get_list_cell(env, channels_left, &channels_head, &channels_tail))
      return enif_make_badarg(env);

    int channels_arity;
    const ERL_NIF_TERM *channels_tuple;
    if (!enif_get_tuple(env, channels_head, &channels_arity, &channels_tuple))
      return enif_make_badarg(env);

    unsigned channels_name_length;
#if (ERL_NIF_MAJOR_VERSION == 2 && ERL_NIF_MINOR_VERSION >= 17) // OTP-26 and later
    if (!enif_get_string_length(env, channels_tuple[0], &channels_name_length, ERL_NIF_LATIN1))
      return enif_make_badarg(env);
#else
    if (!enif_get_list_length(env, channels_tuple[0], &channels_name_length))
      return enif_make_badarg(env);
#endif

    char channels_name[channels_name_length + 1];
    if (enif_get_string(env, channels_tuple[0], channels_name, channels_name_length + 1,
                        ERL_NIF_LATIN1) <= 0)
      return enif_make_badarg(env);

    if (!rosidl_runtime_c__String__assign(&message_p->channels.data[channels_i].name,
                                          channels_name))
      return raise(env, __FILE__, __LINE__);

    unsigned int channels_values_length;
    if (!enif_get_list_length(env, channels_tuple[1], &channels_values_length))
      return enif_make_badarg(env);

    rosidl_runtime_c__float32__Sequence channels_values;
    if (!rosidl_runtime_c__float32__Sequence__init(&channels_values, channels_values_length))
      return enif_make_badarg(env);
    message_p->channels.data[channels_i].values = channels_values;

    unsigned int channels_values_i;
    ERL_NIF_TERM channels_values_left, channels_values_head, channels_values_tail;
    for (channels_values_i = 0, channels_values_left = channels_tuple[1];
         channels_values_i < channels_values_length;
         ++channels_values_i, channels_values_left = channels_values_tail) {
      if (!enif_get_list_cell(env, channels_values_left, &channels_values_head,
                              &channels_values_tail))
        return enif_make_badarg(env);

      double channels_values_i_;
      if (!enif_get_double(env, channels_values_head, &channels_values_i_))
        return enif_make_badarg(env);
      message_p->channels.data[channels_i].values.data[channels_values_i] = channels_values_i_;
    }
  }
...
ERL_NIF_TERM nif_sensor_msgs_msg_point_cloud_get(ErlNifEnv *env, int argc,
  ...
  for (size_t points_i = 0; points_i < message_p->points.size; ++points_i) {
    points[points_i] =
        enif_make_tuple(env, 3, enif_make_double(env, message_p->points.data[points_i].x),
  ...
  for (size_t channels_i = 0; channels_i < message_p->channels.size; ++channels_i) {
    ...
    for (size_t channels_values_i = 0;
         channels_values_i < message_p->channels.data[channels_i].values.size;
         ++channels_values_i) {
      channels_values[channels_values_i] = enif_make_double(
          env, message_p->channels.data[channels_i].values.data[channels_values_i]);
    }

    channels[channels_i] = enif_make_tuple(
        env, 2,
        enif_make_string(env, message_p->channels.data[channels_i].name.data, ERL_NIF_LATIN1),
        enif_make_list_from_array(env, channels_values,
                                  message_p->channels.data[channels_i].values.size));
  }

  return enif_make_tuple(
      env, 3,
      enif_make_tuple(env, 2,
                      enif_make_tuple(env, 2, enif_make_int(env, message_p->header.stamp.sec),
