...
ERL_NIF_TERM nif_std_msgs_msg_multi_array_dimension_type_support(ErlNifEnv *env, int argc,
...
ERL_NIF_TERM nif_std_msgs_msg_multi_array_dimension_create(ErlNifEnv *env, int argc,
                                                           const ERL_NIF_TERM argv[]) {
  ignore_unused(argv);
...
ERL_NIF_TERM nif_std_msgs_msg_multi_array_dimension_destroy(ErlNifEnv *env, int argc,
                                                            const ERL_NIF_TERM argv[]) {
  ...
  std_msgs__msg__MultiArrayDimension *message_p =
      (std_msgs__msg__MultiArrayDimension *)*ros_message_pp;
  std_msgs__msg__MultiArrayDimension__destroy(message_p);

  return atom_ok;
}

ERL_NIF_TERM nif_std_msgs_msg_multi_array_dimension_set(ErlNifEnv *env, int argc,
                                                        const ERL_NIF_TERM argv[]) {
  ...
  if (!enif_get_string_length(env, tuple[0], &label_length, ERL_NIF_LATIN1))
    return enif_make_badarg(env);
#else
  if (!enif_get_list_length(env, tuple[0], &label_length)) return enif_make_badarg(env);
#endif

  char label[label_length + 1];
  if (!enif_get_string(env, tuple[0], label, label_length + 1, ERL_NIF_LATIN1))
    return enif_make_badarg(env);
  if (!rosidl_runtime_c__String__assign(&(message_p->label), label))
    return raise(env, __FILE__, __LINE__);

  if (!enif_get_uint(env, tuple[1], &(message_p->size))) return enif_make_badarg(env);

  if (!enif_get_uint(env, tuple[2], &(message_p->stride))) return enif_make_badarg(env);

  return atom_ok;
}

ERL_NIF_TERM nif_std_msgs_msg_multi_array_dimension_get(ErlNifEnv *env, int argc,
                                                        const ERL_NIF_TERM argv[]) {
  if (argc != 1) return enif_make_badarg(env);
  ...
  return enif_make_tuple(env, 3, enif_make_string(env, message_p->label.data, ERL_NIF_LATIN1),
