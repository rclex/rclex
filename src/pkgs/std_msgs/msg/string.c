...
ERL_NIF_TERM nif_std_msgs_msg_string_set(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (!enif_get_string_length(env, tuple[0], &length, ERL_NIF_LATIN1)) return enif_make_badarg(env);
#else
  if (!enif_get_list_length(env, tuple[0], &length)) return enif_make_badarg(env);
#endif

  char data[length + 1];
  if (enif_get_string(env, tuple[0], data, length + 1, ERL_NIF_LATIN1) <= 0)
    return enif_make_badarg(env);
...
ERL_NIF_TERM nif_std_msgs_msg_string_get(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  return enif_make_tuple(env, 1, enif_make_string(env, message_p->data.data, ERL_NIF_LATIN1));
}
