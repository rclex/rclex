...
ERL_NIF_TERM qos_profile_to_keywordlist(ErlNifEnv* env, rmw_qos_profile_t qos){
  ...
  ERL_NIF_TERM liveliness_lease_duration = enif_make_tuple2(env, atom_liveliness_lease_duration, rmw_time_to_term(env, qos.liveliness_lease_duration));
  ERL_NIF_TERM avoid_ros_namespace_conventions = enif_make_tuple2(env, atom_avoid_ros_namespace_conventions, qos.avoid_ros_namespace_conventions?atom_true:atom_false);

  ERL_NIF_TERM list = enif_make_list9(env,
    history,
    depth,
    reliability,
    durability,
