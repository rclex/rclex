defmodule Mix.Tasks.Rclex.Ros2Test do
  use ExUnit.Case

  test "command_exists?/1" do
    assert Mix.Tasks.Rclex.Ros2.command_exists?("ls")
    refute Mix.Tasks.Rclex.Ros2.command_exists?("non_existent_command")
  end

  @tag skip: "take time"
  @tag :tmp_dir
  test "copy_ros2_resources!/3", %{tmp_dir: tmp_dir_path} do
    Mix.Tasks.Rclex.Ros2.copy_ros2_resources!(tmp_dir_path, "arm64v8", "foxy")
  ...
  test "copy_ros2_impl!/1", %{tmp_dir: tmp_dir_path} do
    Mix.Tasks.Rclex.Ros2.copy_ros2_impl!(
  ...
  test "ros2_docker_image_tag/2" do
    assert "arm64v8/ros:foxy-ros-core" =
             Mix.Tasks.Rclex.Ros2.ros2_docker_image_tag("arm64v8", "foxy")
  end

  test "parse_args/1" do
    assert [arch: "arm64v8", ros2_distro: "foxy"] =
             Mix.Tasks.Rclex.Ros2.parse_args(["--arch", "arm64v8", "--ros2-distro", "foxy"])

    assert [nerves_system: "rpi4", ros2_distro: "foxy"] =
             Mix.Tasks.Rclex.Ros2.parse_args(["--nerves-system", "rpi4", "--ros2-distro", "foxy"])
  end
  ...
  test "create_resources_directory!/2", %{tmp_dir: tmp_dir_path} do
    ...
    ^directory_path =
      Mix.Tasks.Rclex.Ros2.create_resources_directory!(tmp_dir_path, arch, ros2_distro)
