defmodule RclexTest do
  ...
  test "rcl_get_topic_names_and_types" do
    ...
    {:ok, publisher_list} = Rclex.Node.create_publishers(node_list, 'StdMsgs.Msg.String', 'testtopic', :single)
    {:ok, node_list_2} = Rclex.Executor.create_nodes(context, 'test_sub_node', 1)
    {:ok, subscriber_list} = Rclex.Node.create_subscribers(node_list_2, 'StdMsgs.Msg.String', 'testtopic', :single)
    node = hd(node_list)
